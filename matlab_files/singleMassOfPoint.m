...
while t <= t_max
    ...
    for i = 1:3
        if i == 1
            ...
            % Follower behavior with obstacle avoidance and V-formation
            if norm([x_goal, y_goal] - [x(i), y(i)]) > position_accuracy
                all_reached_goal = false;

                % Calculate relative desired position for V-formation
                if i == 2
                ...
                % Calculate Attractive Potential to Desired Position
                ...
                % Update kinematic model for the sub-agent
                omega_ref = Kp_omega * error_theta;
                omega_ref = min(max(omega_ref, -omega_max), omega_max);
